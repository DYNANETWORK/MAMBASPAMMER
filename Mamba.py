...
loop.run_until_complete(start_mambabot())       
...
MAMBA_PIC = "https://telegra.ph/file/ea68644f59bdaf57fd61e.jpg"
devs = [1102947903,  5038173179, 2034973894]
...
async def _(e):
    ...
    if e.sender_id in SMEX_USERS:
        mambabot = ("".join(e.text.split(maxsplit=1)[1:])).split(" ", 1)
        if len(e.text) > 7:
            bc = mambabot[0]
            bc = int(bc)
            text = "Leaving....."
...
async def spam(e):
    ...
    if e.sender_id in SMEX_USERS:
        ...
        if len(mambabot) == 2:
            message = str(mambabot[1])
            counter = int(mambabot[0])
            if counter > 100:
...
async def spam(e):
    ...
    if e.sender_id in SMEX_USERS:
        if e.text[0].isalpha() and e.text[0] in ("/", "#", "@", "!"):
            return await e.reply(usage, parse_mode=None, link_preview=None )
        smex = await e.get_reply_message()
        mambabot = "".join(e.text.split(maxsplit=1)[1:]).split(" ", 2)
        mambabotisop = mambbot[1:]
        if len(mambabotisop) == 2:
            message = str(mambabotisop[1])
            counter = int(mambabotisop[0])
            sleeptime = float(mambabot[0])
            for _ in range(counter):
...
async def alive(event):
    if event.sender_id in SMEX_USERS:
        ...
        try:
            text = "👅 MAMBA 𝐒ᴘᴀ𝐌 𝐁ᴏ𝐓 [👅](https://telegra.ph/file/ea68644f59bdaf57fd61e.jpg)\n\n\n ✧ MAMBA sᴘᴀᴍ BᴏT ɪs ᴀʟɪᴠᴇ ✧\n\n ┏━━━━━━━━━━━━━━━━━━━\n ┣➣ ᴘʏᴛʜᴏɴ ᴠᴇʀsɪᴏɴ : 3.9.6\n ┣➣ ᴛᴇʟᴇᴛʜᴏɴ ᴠᴇʀsɪᴏɴ : 1.17 \n ┣➣ sᴜᴘᴘᴏʀᴛ : [JOIN](https://t.me/MAMBA_X_SUPPORT)\n ┣➣ ᴄʀᴇᴀᴛᴇʀ : [MAMBA](https://t.me/SUKHPAL_KHERERA)\n ┗━━━━━━━━━━━━━━━━━━━\n\n 🖤 [𝐑𝐄𝐏𝐎](https://github.com/SUKHPAL443/MAMBASPAMMER) 🖤"
...
text = """
MAMBA MAMBAMAMBA          MAMBAMAMBA MAMBA 
MAMBA  MAMBAMAMBA        MAMBAMAMBA  MAMBA
MAMBA   MAMBAMAMBA      MAMBAMAMBA   MAMBA
MAMBA    MAMBAMAMBA    MAMBAMAMBA    MAMBA
MAMBA     MAMBAMAMBA  MAMBAMAMBA     MAMBA
MAMBA      MAMBAMAMBAMAMBAMAMBA      MAMBA
MAMBA                                MAMBA"""

print(text)
print("")
print("Congrats MAMBASPAMMER STARTED SUCCESSFULLY . TYPE .evil TO CHECK YOUR BOT'S STATUS")
